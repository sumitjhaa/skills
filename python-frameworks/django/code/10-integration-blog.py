...
routes = [
    (r'^/$', lambda: post_list()),
    (r'^/posts/(?P<slug>[-a-zA-Z0-9_]+)/$', lambda slug: post_detail(slug)),
]

# Simulate requests
for path in ['/', '/posts/python-intro/', '/posts/nonexistent/']:
    matched = False
    for pattern, view in routes:
        m = re.match(pattern, path)
        if m:
            result = view(**m.groupdict()) if m.groupdict() else view()
            print(f"=== {path} ===")
            print(result[:250])
            print()
            matched = True
